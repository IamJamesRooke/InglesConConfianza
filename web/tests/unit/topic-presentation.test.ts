...
import { facetGroup, presentFacet } from "../../src/components/curriculum/topic-presentation";

test("word-building examples emphasize the actual prefix or suffix", () => {
  const facets = CURRICULUM_TOPICS.find((topic) => topic.slug === "transformations")!.facetButtons;
  const hood = presentFacet(facets.find((facet) => facet.collection === "morphology:suffix-hood")!);
  assert.equal(hood.label, "neighborhood");
  assert.equal(hood.before, "neighbor");
  assert.equal(hood.emphasis, "hood");
  const un = presentFacet(facets.find((facet) => facet.collection === "morphology:prefix-un")!);
  assert.equal(un.label, "unhappy");
  assert.equal(un.before, "");
  assert.equal(un.emphasis, "un");
  const survivor = presentFacet(facets.find((facet) => facet.collection === "morphology:suffix-er-or")!);
...
test("presentation keeps each topic's choices distinct and preserves source labels", () => {
  for (const topic of CURRICULUM_TOPICS) {
    ...
    for (const facet of topic.facetButtons) {
      ...
      assert.equal(display.before + display.emphasis + display.after, display.label);
      assert.ok(!labels.has(display.label), `${topic.slug}: duplicate label ${display.label}`);

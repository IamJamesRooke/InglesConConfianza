...
function SentenceDocument({ block, active, onUpdateSentence, onUpdateSpanish, onUpdateAnswer, onUpdateCallout, onAddAnswer, onRemoveAnswer, onAddPiece, onDeletePiece }: {
  ...
  return (
    <section className={`lesson-document-sentence ${isTable ? "table" : ""}`} aria-label={isTable ? "Vocabulary table" : "Sentence"}>
      ...
      {showPrompt && <textarea autoFocus={!block.promptText} className="lesson-document-prompt" value={block.promptText} rows={1} placeholder={isTable ? "Instruction above the word list…" : "Instruction above the exercise…"} onChange={(event) => onUpdateSentence("promptText", event.target.value)} onBlur={(event) => { if (!event.currentTarget.value.trim()) { onUpdateSentence("promptText", null); setShowPrompt(false); } }} />}
      <div className="lesson-document-pieces">
        {block.languageBlocks.map((piece, index) => (
          <div key={piece.id} className={`lesson-document-piece ${activePiece === piece.id ? "active" : ""}`} onFocus={() => setActivePiece(piece.id)}>
            ...
            {piece.callout !== null && <label className="lesson-document-annotation"><span>Hint shown to student</span><input autoFocus={focusCalloutId === piece.id} value={piece.callout} onBlur={(event) => { setFocusCalloutId(null); if (!event.currentTarget.value.trim()) onUpdateCallout(piece.id, null); }} onChange={(event) => onUpdateCallout(piece.id, event.target.value)} placeholder="A small clue the student sees…" /></label>}
            ...
            {piece.acceptedAnswers.slice(1).map((answer, offset) => {
              ...
              return <label key={answerIndex} className="lesson-document-annotation alternative"><span>Also accept</span><input autoFocus={focusAlternativePieceId === piece.id && answerIndex === piece.acceptedAnswers.length - 1} value={answer} onBlur={(event) => { setFocusAlternativePieceId(null); if (!event.currentTarget.value.trim()) onRemoveAnswer(piece.id, answerIndex); }} onChange={(event) => onUpdateAnswer(piece.id, answerIndex, event.target.value)} /><button type="button" aria-label="Remove alternative" onClick={() => onRemoveAnswer(piece.id, answerIndex)}>×</button></label>;
            })}
          </div>
        ))}
        {showTrailingPiece && <div className="lesson-document-piece trailing"><textarea rows={1} data-field="spanish" id={`trailing-${block.id}`} value={draftSpanish} onChange={(event) => { const value = event.target.value; setDraftSpanish(value); if (!composingTrailingRef.current && value.trim()) commitTrailingDraft(value); }} onCompositionStart={() => { composingTrailingRef.current = true; }} onCompositionEnd={(event) => { composingTrailingRef.current = false; if (event.currentTarget.value.trim()) commitTrailingDraft(event.currentTarget.value); }} onKeyDown={handleTrailingKey} placeholder={isTable ? "Add a row…" : "Add another blank…"} lang="es" aria-label={isTable ? "New row, Spanish" : "New sentence piece, Spanish"} /><span aria-hidden="true">Tab to add the English</span></div>}
      </div>
      {(showHelper || block.answerFeedback !== null) && <div className="lesson-document-slide-notes">
        {showHelper && <label className="lesson-document-annotation"><span>Help on request</span><textarea autoFocus={!block.helperText} value={block.helperText} onChange={(event) => onUpdateSentence("helperText", event.target.value)} onBlur={(event) => { if (!event.currentTarget.value.trim()) { onUpdateSentence("helperText", null); setShowHelper(false); } }} placeholder="Shown when the student asks for help…" /></label>}
        {block.answerFeedback !== null && <label className="lesson-document-annotation"><span>After correct answer</span><textarea autoFocus={focusNewSuccess} value={block.answerFeedback} onBlur={(event) => { setFocusNewSuccess(false); if (!event.currentTarget.value.trim()) onUpdateSentence("answerFeedback", null); }} onChange={(event) => onUpdateSentence("answerFeedback", event.target.value)} placeholder="A short message after they get it right…" /></label>}
      </div>}
      {(!showPrompt || !showHelper || block.answerFeedback === null) && <details className="lesson-document-options">

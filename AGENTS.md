...
- The immediate curriculum problem is complete identification of **Core Functional Language**. `core` concepts must be explicitly taught; `supporting` language may be taught incidentally around the core focus; `reference` material is retained but is not a current teaching target.

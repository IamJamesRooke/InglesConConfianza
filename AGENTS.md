...
- The former mappings, cognates, vocabulary, transformations, and structure/verb-forms source trees have been captured losslessly in PostgreSQL and retired. Their immutable evidence is in `web/prisma/seed-data/curriculum-sources.json`; teachable content lives in the one canonical `curriculum_concepts` table and is queried through bilingual search and collections.

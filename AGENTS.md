...
- The experimental curriculum graph has been retired. The current machine-readable discovery store is `web/data/curriculum.json`; `Contenido del curso` renders its mappings and supports inline editing, deletion, bilingual search, collection filters, and teaching-priority filters.
...
- The immediate curriculum problem is complete identification of **Core Functional Language**. `core` is an elite, highly selective tier: every Core concept needs a strong justification that learners must explicitly master it to function in English. `supporting` is also selective and is reserved for extremely useful, broadly reusable language taught around the Core focus. `reference` covers independently useful but non-everyday, situational, secondary, or readily inferable material that is retained for retrieval rather than active teaching.
- The workflow still being refined is a small-batch verb-family audit. The intended loop is: choose one high-utility Spanish verb, inspect all relevant material under `docs/curriculum`, enumerate independently teachable mappings and meaning-changing forms, propose mappings with examples, collections, and curriculum roles, obtain owner confirmation—especially for Core candidates—then migrate and validate the approved batch.

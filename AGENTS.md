...
- The former mappings and cognates source trees have been captured losslessly in PostgreSQL and retired. Their immutable evidence is in `web/prisma/seed-data/curriculum-sources.json`; structured cognates are in `cognates.json`. Query cognates with `npm run curriculum:cognates:query -- --search <text>` from `web/`.

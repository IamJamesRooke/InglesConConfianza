...
- RUN EVERY CHECK IN THE FOREGROUND WITH A TIMEOUT. Never `run_in_background` and wait — completion notifications do not reach subagents; a session that waits stalls until a human notices.
- Two lanes. **Cosmetic** (CSS, layout, copy, presentation): work in the main tree, read only the style section you touch, run the CSS lint, ONE targeted Playwright spec and take ONE "after" screenshot — nothing more; the coordinator runs the heavy gate once at the end of the round. **Behaviour/data/keyboard/server** changes use the full list below.
- Verify with: `npm run test:unit` (scope with `-- tests/unit/<file>.test.ts` when possible), `npx eslint <touched files>`, `npx tsc --noEmit`, `npm run lint` (includes the CSS lint), and **only the Playwright specs for what you touched** (`npx playwright test tests/ux/<spec>.spec.ts`; Playwright runs its own dev server on an isolated port against a throwaway lessons file — safe anytime; set `UX_CHECK_PORT` if another session is running). The coordinator runs the full `npm run ux:check` and `npm run build` only when the owner asks for a gated run — never as part of a task. Cosmetic sessions (CSS, copy, layout) run `npm run lint` and take ONE screenshot, nothing else; behaviour fixes run the touched unit test and at most ONE Playwright spec.

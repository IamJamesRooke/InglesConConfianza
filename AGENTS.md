...
- The active curriculum objective is completeness-first migration of everything useful under `docs/curriculum` into PostgreSQL, beginning with `docs/curriculum/mappings/english-to-spanish`. Rewrite that reverse-direction evidence into canonical Spanish-first concepts, prevent duplicates, preserve source dispositions, and defer broad curation until migration is complete.
...
- Process mapping sources in small English-hub batches. The loop is: inventory every file, extract and normalize useful concepts, compare the proposed batch with PostgreSQL, import through Review, migrate transactionally, verify database/snapshot parity, record every file's disposition, then consume the source and commit.

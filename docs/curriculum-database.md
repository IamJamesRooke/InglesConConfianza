...
During curation, apply collection changes from a reviewed manifest via `curriculum:collections:apply` (DELETE / MERGE / RENAME ops), recorded under `docs/curation/archive/manifests/`. Merge spelling variants and collections that encode the same idea; give a new tag a facet. Do not introduce a second tag system until demonstrated needs exceed collections.
...
5. Apply the batch from a reviewed TSV manifest (`curriculum:roles:apply`, `curriculum:concepts:apply`), recorded under `docs/curation/archive/manifests/`. Export updated immutable snapshots, then run database regression tests and snapshot parity verification.

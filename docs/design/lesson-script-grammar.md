...
| `[[img: file.svg \| alt text]]` | Sets the slide's image (`ExplanationBlock.image`) — its own line, anywhere among the slide's paragraphs (parsing accepts it in any position). `file` is a bare filename under `web/public/lesson-media/`; `alt` is the Spanish alt text. `printScript` emits it as the FIRST line of the explanation (owner, 2026-09-18: the image renders above the text on the learner side, and the script mirrors that). A second `[[img: ]]` line in the same slide is a parse error with the line number; an invalid filename (not matching `^[a-z0-9][a-z0-9-]*\.(svg\|png\|jpg\|jpeg\|webp)$`) is also a parse error. |

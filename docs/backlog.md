...
- [ ] Small: the first-visit gate reaches /bienvenida through a loading shell + client redirect (streamed 200, not a 307) — remove the flicker; the phone answer slot is wider than its answer; swap the dark UK portrait for one facing the learner; full `ux:check` has not been run since the 17th (run once friends have the link, with the owner's word).

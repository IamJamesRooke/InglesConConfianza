...
- [ ] **NEXT UP — ElevenLabs voices (owner, 2026-09-18 night: "SO MUCH better, that has to be done").** Decided, no longer "later". Plan before building (one short plan for the owner, then one session): which voices (a male Latin American Spanish narrator — Colombian if one exists — plus the US man and UK woman for English; owner picks by ear from 2–3 candidates each), how two-voice explanations are made (ElevenLabs has no SSML `<voice>` switching: generate one segment per voice and concatenate, keeping the pauses and the slower narrator pace the owner asked for), pronunciation of respellings and single letters, cost per full regeneration against `docs/design/audio-provider-costs.md`, and the key as `ELEVENLABS_API_KEY` in `web/.env` only (never in chat, never on Vercel). Technical seam: swap `synthesize()` / `synthesizeSsml()` in `web/src/lib/audio/generate-clips.ts` behind a provider switch so Google stays as the fallback; clip file names are content hashes of text + voice, so a new voice id regenerates everything and `npm run assets:prune -- --apply` clears the old clips. Tests must never call the real service (blank the key in `playwright.config.ts` like the Google one). Then "publish the course".
- [ ] Look at (owner, 2026-09-18 night — nothing urgent, all noted at close of day):
  - Two older working copies beside the project that predate today and were left untouched: `/data/MEGA/Projects/icc-wt-meta` (detached HEAD 26ca718b) and `/data/MEGA/Projects/icc-wt-syllabus` (branch `design/stacked-pills`, 0baaa736). Check whether either holds work worth keeping; if not, remove with `git worktree remove` and delete the branch.
  - Vercel: add `FEEDBACK_GITHUB_TOKEN` (sensitive) and redeploy before anyone gets the link; decide on `ADMIN_SECRET` (left off → public admin is 404).
  - Rotate the Google TTS key (it appeared in a session log once). Moot once ElevenLabs replaces it, but rotate anyway if Google stays as fallback.
  - Owner's pass over the onboarding wording in his own voice; his photo on the first slide; three orphan words at phone width ("punto." twice, "that," once).
  - Live with the site on a phone for a few days and send ONE batched list; then one trusted person before "friends".
  - A journal entry for today was written to `~/Documents/ingles-con-confianza-2026-09-18.md` (outside the repo) — keep or delete.
  - One screenshot containing a personal photo sits in Claude Code's local image cache (`~/.claude/image-cache/`); never published; delete if wanted.
...
- [x] ElevenLabs voices — PROMOTED 2026-09-18, see "Next up" near the top (was: keep Google TTS for now).

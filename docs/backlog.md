...
> Quick handoff: PostgreSQL is the canonical curriculum store; Lesson Builder remains JSON-backed. The former mappings, cognates, vocabulary, and transformations trees are preserved losslessly in PostgreSQL and its immutable seed snapshot, so normalization and curation now work from the database.
...
- [ ] Plan the same inventory-to-PostgreSQL process for past and past participle and structure.

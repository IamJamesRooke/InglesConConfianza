...
- The active objective is completeness-first migration of every useful concept under `docs/curriculum` into PostgreSQL, beginning with the remaining English-to-Spanish mapping tree. Curation, promotion, and demotion follow later. Missing useful language is expensive; temporarily retaining uncertain but valid language as Reference is cheap.
...
- Work in small source-hub batches. For English-to-Spanish hubs, treat the English source as evidence and rewrite every useful item as a neutral Spanish-first concept. Audit basic complements, infinitive and person-plus-action constructions, independently useful meanings, meaning-changing forms, common fixed expressions, and nonliteral English realizations.
...
- Treat migration-source review as a one-look process. As soon as every useful part of a source file is represented in the database, the review inbox, or a deliberately preserved future-family source, delete the consumed file instead of archiving it. For a mixed file, split or remove only the consumed sections and retain the genuinely unprocessed material. Git history is the recovery mechanism; routine future audits must not reopen consumed sources.
...
- Before consuming a source hub, account for every file in its migration manifest and run the review batch through database-backed duplicate preflight. A source file may be marked complete only when its useful material is migrated, retained in review history, deliberately classified as non-concept teaching material, or moved to another canonical pillar.

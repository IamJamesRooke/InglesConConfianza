...
- Owner-approved close-form families may use one human-facing owner even when the forms are not inflections: **ante** lives under `antes`; **abajo** and **debajo** under `bajo`; **ahí** under `allí`; **cuán** under `cuánto`; **tan** under `tanto`; and accented **sí** under `si`. Use this exception only when the visible relationship improves retrieval, not for words that merely resemble one another.
